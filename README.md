...
- 更正只有一篇文章时的判断错误
- 更正 Vue 错误复用组件一次的错乱
- 让 Markdown 中代码块在空间不够时出现滚动条

### 对于老版本升级

- 已经发布到 npm,只需要把 package.json 中的主题版本号改为 1.0.2,然后删除 node_modules 文件夹,重新运行 npm i 或者最好用 cnpm i 即可
...
- 享用 vuepress 的 中代码块在空间不够时出现滚动条 拓展 , 例如在 Markdown 中 使用 Vue,自定义文章摘要
- 理论上非常好的 SEO
