...
- 参照原主题改进了 ssr.html 模板文件
- 成功在文章页滚动监听节流,最后效果就是快速滚动不再导致页面卡帧,目录高亮又能准确快速

### 对于老版本升级

- 已经发布到 npm,只需要把 package.json 中的主题版本号改为 1.0.1,然后删除 node_modules 文件夹,重新运行 npm i 或者最好用 cnpm i 即可
